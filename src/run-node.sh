...
# Many thanks to https://github.com/sindresorhus/alfy/blob/master/run-node.sh
...
if has_node; then
  ...
  echo $'{"items":[{"title": "Couldn\'t find the `node` binary", "subtitle": "Please symlink it to `/usr/local/bin`"}]}'
fi
